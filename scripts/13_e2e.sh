...
trace_features=()
for argument in "$@"; do
  if [[ "$argument" == "--function-trace" ]]; then
    trace_features=(--features function-trace)
  fi
done
python3 scripts/5_probe_guard.py \
  ...
  bash -c 'cargo build --release --locked -j 1 --bin ascii-renderer "$@" && cargo build --release --locked -j 1 --example 1_native_terminal' _ "${trace_features[@]}" \
