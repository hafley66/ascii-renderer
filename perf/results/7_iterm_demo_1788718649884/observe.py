...
record('video_requested')
video=subprocess.Popen(['/usr/sbin/screencapture','-x','-v','-V','40',f'-l{W}',str(D/'visible.mov')],stdout=(D/'capture.log').open('w'),stderr=subprocess.STDOUT)
time.sleep(1)
key('a')
...
key('q')
# Capture the real display independently of the renderer while quit is pending.
for i in range(16):
    screenshot(f'quit-{i:02}.png')
    time.sleep(.05)
...
try:video.wait(timeout=45)
except subprocess.TimeoutExpired:video.terminate();video.wait(timeout=5)
record('video_finished',code=video.returncode)
