# perf-instr-sky: wrap painters in measure_layer (pass 1 of 2)
...
src/modes_sky.rs, src/modes_tree.rs, src/modes_creatures.rs, src/cli_city.rs

## Modes to instrument (registry names, use them verbatim as the first measure_layer argument)

hypercube, flux, fireworks, murmuration, lanterns, tide (src/modes_sky.rs); delta, fullmetal-eyes (src/modes_tree.rs); snakes (src/modes_creatures.rs); elevator, ferris (src/cli_city.rs)
...
Before any cargo command run `export CARGO_TARGET_DIR=$PWD/target-perf-instr-sky` in your shell (the worktree is $PWD). Never cd into another checkout.
