...
The rebuilt default executable was tested through `scripts/13_e2e.sh --headless --directory perf/results/50_demo_e2e`. Workflow, recorded-knob 400×200 and max-knob 400×200 cases passed. The deliberate backpressure case failed: the `-` knob input applied after 1004.89 ms against a 250 ms gate, while `q` restored terminal mode in 2.60 ms. Overall suite status is failed. GUI painting and the focused Rust unit tests were not executed in this run.

The first full-suite attempt (`49_demo_e2e`) exposed an artifact-scanning race in the watchdog: atomic replacement renamed `native-terminal-stats.tmp` between directory enumeration and `stat`. The guard now retries a disappeared-file scan once and still stops on a second failure. All time, memory and disk thresholds remain unchanged. Its four tests, including deterministic rename-race coverage and process-tree termination, passed; the three evidence-reader tests passed.
