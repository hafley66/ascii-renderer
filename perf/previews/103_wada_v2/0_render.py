"""Bounded color review for wada-v2 using the repository's serialized Cell format.
Braille cells are decoded to their eight subcell dots instead of a font glyph.

python3 perf/previews/103_wada_v2/0_render.py BINARY OUTPUT_DIRECTORY
Adapted from perf/previews/102_wada/0_render.py; no terminal playback.
"""
import json
import os
...
CW, CH = 8, 16
DOTS = [(0, 0, 0x01), (0, 1, 0x08), (1, 0, 0x02), (1, 1, 0x10),
        (2, 0, 0x04), (2, 1, 0x20), (3, 0, 0x40), (3, 1, 0x80)]
...
for name, w, h, seed, t, theme, values in cases:
    ...
    command = [binary, seed, 'wada-v2', theme] + values
    ...
    image = Image.new('RGB', (w * CW, h * CH + 28), (2, 7, 13))
    draw = ImageDraw.Draw(image)
    draw.text((8, 4), f'wada-v2 {name}', font=font, fill=(156, 179, 188))
    for i, (ch, fg, bg) in enumerate(cells):
        code = int(ch)
        color = lambda s: tuple(map(int, s.split(','))) if s != 'x' else (2, 7, 13)
        x, y = (i % w) * CW, (i // w) * CH + 28
        draw.rectangle((x, y, x + CW - 1, y + CH - 1), fill=color(bg))
        if 0x2800 <= code < 0x2900:
            bits = code - 0x2800
            for row_i, col_i, bit in DOTS:
                if bits & bit:
                    dx = x + 1 + col_i * 4
                    dy = y + 1 + row_i * 4
                    draw.ellipse((dx, dy, dx + 2, dy + 2), fill=color(fg))
        elif code != 32:
            draw.text((x, y - 1), chr(code), font=font, fill=color(fg))
