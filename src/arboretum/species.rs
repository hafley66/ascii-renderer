...
pub(super) fn grow_species(
    ...
    let budget =
        (3.0 + (budget_full.max(2) as f32 - 3.0) * ease_in_out(grow.clamp(0.0, 1.0))) as i32;
    let budget = budget.max(2);
    match genome.style {
        TreeStyle::Conifer => conifer(
            grid,
            rx,
            ry,
            budget,
    ...
    for (i, &(nx, ny)) in nodes.iter().enumerate() {
        ...
        for dx in -half..=half {
            ...
            if rng.random::<f32>() < foliage * 0.92 {
                ...
                let c = if rng.random::<f32>() < 0.3 {
                    lighten(cols.leaf, 15)
                } else {
                    cols.leaf
                };
...
fn broadleaf(
    ...
    for (i, &(nx, ny)) in nodes.iter().enumerate() {
        ...
        let run =
            ((budget as f32) * (0.12 + 0.16 * i as f32 / n as f32) * genome.spread).max(2.0) as i32;
        for _ in 0..run {
            bp.step(
                grid,
                if side > 0 {
        ...
        for dy in -r.ceil() as i32..=r.ceil() as i32 {
            for dx in -r.ceil() as i32..=r.ceil() as i32 {
                ...
                if rng.random::<f32>() < p {
                    ...
                    let c = if rng.random::<f32>() < 0.35 {
                        lighten(cols.leaf, 18)
                    } else {
                        cols.leaf
                    };
...
fn willow(
    ...
    for s in 0..strand_n {
        ...
        let sx =
            nodes.last().map(|&(ax, _)| ax).unwrap_or(rx) + srng.random_range(-spread_w..=spread_w);
        let sy = nodes.last().map(|&(_, ay)| ay).unwrap_or(ry) + srng.random_range(0..2);
        ...
        for k in 0..len {
            ...
            if rng.random::<f32>() < genome.leafage * foliage * 0.45 && blank_at(grid, x + 1, y) {
...
fn madness(
    ...
    if rng.random::<f32>() < genome.leafage * foliage {
        ...
        set(
            ...
            pen.y - 1,
            g,
            lighten(cols.leaf, rng.random_range(0..30)),
        );
    }
...
fn pleach(
    ...
    for i in 0..arm_n {
        for side in [1i32, -1] {
            let start_i =
                (full_n as f32 * (0.25 + 0.55 * (i as f32 + 1.0) / arm_n as f32)) as usize;
            if start_i >= n {
                continue;
            }
            ...
            let reach = ((budget_full as f32) * 0.16 * (i as f32 + 1.0) / arm_n as f32
                * genome.spread)
                .max(2.0) as i32;
    ...
    while x <= xmax {
        let inb = ceiling >= 0
            && x >= 0
            && (ceiling as usize) < grid.len()
            && (x as usize) < grid[0].len();
        let on_line = if inb {
            grid[ceiling as usize][x as usize].ch
        } else {
            ' '
        };
        if on_line == ' ' || on_line == '│' {
...
fn uzumaki(
    ...
    for k in 0..trunk_h {
        ...
        let gx = tx
            + (spin * (k as f32 * 0.7 + phase + std::f32::consts::PI + sway * 3.0).sin() * amp)
                .round() as i32;
    ...
    for t in 0..tendril_n {
        let (sx, sy) = nodes[((t as f32 / tendril_n as f32 * nodes.len() as f32 * 0.7) as usize)
            .min(nodes.len() - 1)];
        ...
        for s in 0..len {
            ...
            tp.step(grid, dir);
            if rng.random::<f32>() < genome.leafage * foliage * 0.35
                && blank_at(grid, tp.x, tp.y - 1)
            {
...
fn cypress(
    ...
    set(
        grid,
        tx + (lean * h as f32 * h as f32 * 0.002).round() as i32,
        ty - h,
        '╷',
        lighten(cols.branch, 30),
    );
}
...
mod tests {
    ...
    fn species_grid(style: TreeStyle) -> String {
        ...
        let genome = TreeGenome {
            style,
            ..TreeGenome::roll(&mut r)
        };
    ...
    fn uzumaki_trunk_coils_and_crown_vortexes() {
        ...
        assert!(
            cols_with_wood.len() >= 2,
            "helix trunk should span columns: {:?}",
            cols_with_wood
        );
    ...
    fn pleach_tips_meet_one_row() {
        ...
        assert!(
            tip_rows.len() >= 2,
            "expected several trained tips, got {}",
            tip_rows.len()
        );
        assert_eq!(
            tip_rows[0],
            tip_rows[tip_rows.len() - 1],
            "tips must share one row: {:?}",
            tip_rows
        );
        assert_eq!(
            topmost, tip_rows[0],
            "nothing may rise above the shared ceiling"
        );
    }
    ...
    fn babel_grows_sane_then_crazy() {
        ...
        assert!(
            sane.contains('│'),
            "lower trunk should be ordinary wood: {:?}",
            sane
        );
        ...
        assert!(
            crazy.contains('◌')
                ...
                || crazy.contains('◆')
                || crazy.contains('✦'),
            "crown zone should hold impossible things: {:?}",
            crazy
        );
    }
}
