...
fn elevator_waiting(seed: u64, floor: u64, pc: u64, crowd: f32) -> usize {
    let h = emix(
        seed ^ floor
            ...
            .wrapping_add(pc.wrapping_mul(0x2722_0A95)),
    );
...
pub(crate) fn draw_elevator(
        ...
        for lift in &lift_setups {
            ...
            for k in 0..lift.seq.len() - 1 {
                ...
                if tau < travel_end {
                    ...
                    arriving = Some((
                        b,
                        elevator_waiting(seed, b as u64, pc_arr, crowd) > 0 && q < 0.55,
                    ));
                    acc = dwell_end;
                    break;
                }
    ...
    measure_layer("elevator", "floor_paint", || {
        for f in 0..floors {
            let ry = row_y(f) as usize;
            let open_here = frames
                .iter()
                .find_map(|fr| fr.open_floor.filter(|&ff| ff == f).map(|_| fr));
            match open_here {
                Some(fr) => {
                    let q = fr.open_q;
        ...
        for (lift, fr) in lift_setups.iter().zip(frames.iter()) {
            ...
            let cells: [char; 3] = if fr.open_floor.is_some() {
                ...
                if q < 0.25 {
                    ...
                    [
                        '·',
                        if fr.arriving.map(|(_, b)| b).unwrap_or(false) {
                            '☻'
                        } else {
                            '·'
                        },
    ...
    let draw_eye = |grid: &mut Grid,
        ...
        for dy in -ry - 1..=ry + 1 {
            for dx in -rx - 1..=rx + 1 {
                ...
                let idx = dx - gaze_x;
                let im =
                    (idx as f32 / iris_rx as f32).powi(2) + (idy as f32 / iris_ry as f32).powi(2);
                if im <= 1.0 {
                    ...
                    if pm <= 1.0 {
                        ...
                        let ch =
                            match (style + dx.unsigned_abs() as usize + dy.unsigned_abs() as usize)
                                % 7
                            {
                                0 => '◌',
                                1 => '○',
                    ...
                    let ch =
                        match (style + dx.unsigned_abs() as usize * 2 + dy.unsigned_abs() as usize)
                            % 9
                        {
                            0 => '·',
                            1 => '∙',
...
pub(crate) fn cli_eyes2(
    ...
    for i in 0..eye_count {
        ...
        let angle = std::f32::consts::PI + t * std::f32::consts::PI + rng.random_range(-0.20..0.20);
...
pub(crate) fn cli_metro(
    ...
    for li in 0..line_count {
        ...
        if li == 0 && line_count >= 3 {
            let x0 =
                rng.random_range(width as i32 / 8..(width as i32 / 3).max(width as i32 / 8 + 1));
            let x1 = rng.random_range(
                width as i32 * 3 / 5..(width as i32 * 7 / 8).max(width as i32 * 3 / 5 + 1),
            );
            let y0 = rng.random_range(2..(height as i32 / 3).max(3));
            let y1 = rng.random_range(
                height as i32 * 3 / 5..(height as i32 - 2).max(height as i32 * 3 / 5 + 1),
            );
    ...
    for _ in 0..(width / 14).max(3) {
        ...
        let btop = match kind {
            ...
            2 => {
                ...
                while tw >= 3 && bottom > 3 {
                    ...
                    for by in (t_top + 1..bottom).step_by(2) {
                        for bx in (tx + 1..(tx + tw).min(width).saturating_sub(1)).step_by(2) {
                            let on = rng.random_range(0..100) < lit;
                            grid[by][bx] = Cell::new('▪', if on { win_on } else { win_off });
                        }
                    }
            ...
            4 => {
                ...
                let h = rng.random_range((height * 3 / 5).max(5)..(height * 5 / 6).max(6));
                let top = horizon.saturating_sub(h);
                for bx in x..x + w {
                    grid[top][bx] = Cell::new('▄', near);
                    for by in top + 1..horizon {
...
pub(crate) fn cli_jelly2(
    ...
    for _ in 0..count {
        ...
        if bell == 4 {
            ...
            for ty in [cy - 1, cy, cy + 1] {
                ...
                let mut prev = 0i32;
                for j in 1..=len {
                    let sway = ((j as f32 * 0.5 + phase).sin() - phase.sin()) * amp;
        ...
        if tstyle == 3 {
            for dx in [-r + 1, r - 1] {
                let len: i32 =
                    rng.random_range((height as i32 / 4).max(3)..(height as i32 / 2).max(4));
                let phase = rng.random_range(0.0f32..6.3);
                let mut prev = 0i32;
                for j in 1..=len {
    ...
    for _ in 0..count / 2 + 1 {
        ...
        jput!(x, y, '∩', dim);
        let tail = if rng.random_range(0..2) == 0 {
            '¦'
        } else {
            '\''
        };
...
pub(crate) fn cli_elevator(
    ...
    draw_elevator(
        &mut grid, width, height, seed, &palette, &mut rng, t_anim, lifts, speed, crowd,
    );
    (grid, false)
}
    ...
    measure_layer("ferris", "gondolas", || {
        for k in 0..n {
            ...
            let occupied = emix(
                seed ^ (k as u64)
                    .wrapping_mul(0x9E37)
                    .wrapping_add((rev as i64 as u64).wrapping_mul(0xBF58)),
            ) % 100
                < 72;
            ...
            pp_put(
                ...
                cy2,
                if occupied { '☻' } else { '·' },
                if occupied { rider_c } else { darken(col, 40) },
            );
            pp_put(grid, px.round() as i32 + 1, cy2, '▌', col);
        }
    });
...
pub(crate) fn cli_ferris(
    ...
    let speed: f32 = args
        ...
        .unwrap_or_else(|| param_f32("SPEED", 1.0));
    draw_ferris(
        &mut grid, width, height, seed, &palette, &mut rng, t_anim, radius, gondolas, speed,
    );
    (grid, false)
}
