    ...
    if lvl == max_lvl {
        if !is_root {
            for _ in 0..5 {
                ...
                leaves.push(BLeaf {
                    z: (end.0 + r * a2.cos(), end.1 + r * a2.sin()),
                    phase: rng.random::<f32>(),
                    tex: rng.random_range(0..3u32) as u8,
                });
            }
        }
        return;
    ...
    if k.rings > 0.0 {
        for r in &c.rings {
            ...
            for i in 0..=n {
                ...
                if side_of(z) {
                    let col = scale(
                        mix(c.eth_rgb, c.eth_hi, pulse * 0.7),
                        0.3 * k.rings * (0.5 + 0.5 * pulse),
                    );
                    if let Some(p) = prev {
                        let mut cnt = 0u32;
