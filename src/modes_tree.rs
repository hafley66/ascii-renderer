    ...
    for (bi, &r) in [0.92f32, 0.62].iter().enumerate() {
        ...
        for i in 0..ins_n {
            ...
            pp_put(
                ...
                p.0,
                p.1,
                runes[(i + seed as usize + bi * 3) % runes.len()],
                lighten(gold, 8),
            );
        }
    }
    ...
    for (ti, verts) in tier_verts.iter().enumerate() {
        ...
        for (i, &(nx, ny)) in verts.iter().enumerate() {
            ...
            draw_node_eye(
                ...
                gy,
                darken(lid, 6),
                shift_hue(iris, (i as f64 * 40.0 + ti as f64 * 17.0) % 360.0),
            );
            pp_put(
                grid,
                nx,
                ny + ry + 1,
                runes[(i + ti) % runes.len()],
                lighten(gold, 12),
            );
        }
    }
    ...
    for row in 0..rows {
        for col in 0..cols {
            ...
            if (row + col + seed as usize) % 2 == 0 {
                draw_flower(
                    grid,
                    (cx + spread).min(width.saturating_sub(1)),
                ...
                draw_fruit(
                    grid,
                    cx.saturating_sub(spread / 2),
                    (canopy_y + 3).min(height.saturating_sub(1)),
                    kind % 5,
...
pub(crate) fn draw_forest_pp(
    ...
    for _ in 0..(width / 2) {
        ...
        let ch = if rng.random_range(0..4) == 0 {
            '✦'
        } else {
            '·'
        };
    ...
    clear(
        grid,
        fg_a.saturating_sub(8),
        fg_a + 8,
        ground_y.saturating_sub(14),
        ground_y,
    );
    grow_tree(
        grid,
    ...
    clear(
        grid,
        fg_c.saturating_sub(9),
        fg_c + 9,
        ground_y.saturating_sub(16),
        ground_y,
    );
    draw_willow(
        grid,
            ...
            for dy in top..=bottom {
                ...
                if dy == top || dy == bottom {
                    ...
                    if im <= 0.16 {
                        ...
                        let ang = (iy.atan2(ix) + std::f32::consts::TAU) % std::f32::consts::TAU;
                        let fiber =
                            ((ang / (std::f32::consts::TAU / 16.0)).round() as i32) % 2 == 0;
                        let ch = if im > 0.78 {
                            '○'
                        } else if fiber {
                            '╎'
...
pub(crate) fn draw_fa6(
    ...
    let (core_rx, core_ry, cx, cy, seed_phase, chambers) = measure_layer("fa6", "chambers", || {
        ...
        while chambers.len() < chamber_count {
            let split_idx = chambers
                ...
                .max_by_key(|(_, c)| c.x1.saturating_sub(c.x0) * c.y1.saturating_sub(c.y0))
                .map(|(i, _)| i);
    ...
    measure_layer("fa6", "chamber_paint", || {
        for (ci, chamber) in chambers.iter().enumerate() {
            ...
            let area =
                chamber.x1.saturating_sub(chamber.x0) * chamber.y1.saturating_sub(chamber.y0);
            ...
            let tag = format!(
                "{:02X}",
                (seed as usize + ci * 29 + chamber.depth * 11) & 0xff
            );
    ...
    measure_layer("fa6", "finale", || {
        ...
        for i in 0..tick_count {
            let a =
                phase * if i % 2 == 0 { 1.0 } else { -0.7 } + i as f32 * TAU / tick_count as f32;
        ...
        let left = (
            cx - (core_rx * 0.17).round() as i32,
            cy + (core_ry * 0.20).round() as i32,
        );
    ...
    measure_layer("delta", "strokes", || {
        for i in 0..n {
            ...
            if rbow > 0.0 {
                ...
                let hue = ((nd.depth as f32 / 7.0) * 280.0 + (ex / width as f32) * 80.0)
                    .rem_euclid(360.0);
