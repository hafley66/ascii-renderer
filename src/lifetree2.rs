...
#[tracing::instrument(level = "trace", target = "ascii_renderer::functions", skip_all)]
fn build(w: usize, h: usize, seed: u64, palette: &[Color; 5], k: &VeilKnobs) -> Cached {
...
#[inline]
#[tracing::instrument(level = "trace", target = "ascii_renderer::functions", skip_all)]
fn frame(grid: &mut Grid, c: &Cached, t: f32, k: &VeilKnobs) {
    ...
    if k.motes > 0 {
        ...
        for m in &c.motes {
            ...
            let x =
                (m.x0 + m.amp * (life * m.freq * 6.2832 + m.phase * 6.2832).sin()).round() as i32;
            if !is_eth(x, y) {
                continue;
            }
    ...
    for f in 0..k.flock {
        ...
        for b in 0..7 {
            let bx = (fx + heading * (b as f32 - 3.0) * 2.2 + (ts * 0.9 + b as f32).sin()).round()
                as i32;
            let by = (fy + ((b as f32 - 3.0).abs() * 0.7) + 0.6 * (ts * 1.7 + b as f32 * 1.3).sin())
                .round() as i32;
...
mod tests {
    ...
    fn run(w: usize, h: usize, seed: u64, t: f32) -> String {
        ...
        let k = VeilKnobs::from_env();
        draw_lifetree2(&mut g, w, h, seed, &p, t, &k);
        g.iter()
    ...
    #[test]
    #[tracing::instrument(level = "trace", target = "ascii_renderer::functions", skip_all)]
    fn snapshot_lifetree2_small() {
        insta::assert_snapshot!("lifetree2_80x24", run(80, 24, 42, 0.0));
    }

    #[test]
    #[tracing::instrument(level = "trace", target = "ascii_renderer::functions", skip_all)]
    fn snapshot_lifetree2_wide() {
        insta::assert_snapshot!("lifetree2_120x40", run(120, 40, 42, 0.0));
    }
    ...
    fn seasons_cycle_leaf_visibility() {
        ...
        let count = |t: f32| {
            ...
            let p = crate::color::named_theme("moss").unwrap();
            draw_lifetree2(&mut g, 100, 32, 42, &p, t, &k);
            g.iter()
                .flatten()
    ...
    fn frame_cost_is_flat() {
        ...
        let k = VeilKnobs::from_env();
        draw_lifetree2(&mut g, 200, 60, 42, &p, 0.0, &k);
        let start = std::time::Instant::now();
        for i in 1..=200 {
            draw_lifetree2(&mut g, 200, 60, 42, &p, i as f32 * 0.06, &k);
        }
        let per = start.elapsed().as_secs_f64() / 200.0;
        eprintln!("lifetree2 frame 200x60: {:.3}ms", per * 1000.0);
        assert!(
            per < 0.004,
