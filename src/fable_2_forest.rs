...
const ATMOS_KINDS: [Atmos; 6] = [Atmos::Mist, Atmos::Fireflies, Atmos::Leaves, Atmos::Rain, Atmos::Snow, Atmos::Birds];
...
fn build_scene(key: Key, k: &ForestKnobs) -> Scene {
    ...
    let weights: Vec<f32> = (0..SPECIES.len()).map(|i| if i == dom { 0.5 } else if i == sec { 0.28 } else { 0.22 / (SPECIES.len() as f32 - 2.0) }).collect();
    ...
    let gk = GrowKnobs { fruit: key.fruit, branch: 0.8, detail: 1.0, roots: 1.0 };
    ...
    for li in 0..layers {
        ...
        while layer_trees.len() < n && attempts < n * 12 {
            ...
            layer_trees.push(Tree { sprite, x: x0, y: root_y as i32 - (th as i32 - 1), lfrac, phase });
        }
    ...
    let atmos = if key.atmos >= 1.0 { ATMOS_KINDS[(key.atmos.round() as usize - 1).min(5)] } else { ATMOS_KINDS[rng.random_range(0..6u32) as usize] };
    let accent = key.palette[3];
    let pale = key.palette[4];
    let (atm_slots, parts) = match atmos {
        Atmos::Mist => ([pal.intern(hsl_to_rgb(sky_hue, 0.25, 0.22)), pal.intern(hsl_to_rgb(sky_hue, 0.25, 0.3)), 0], Vec::new()),
        Atmos::Fireflies => {
            let n = ((w * h) / 900).clamp(8, 600);
            let ps = (0..n)
                .map(|_| Particle {
                    x: rf(&mut rng) * w as f32,
                    y: horizon as f32 * 0.6 + rf(&mut rng) * (h as f32 * 0.95 - horizon as f32 * 0.6),
                    phase: rf(&mut rng),
        ...
        Atmos::Leaves => {
            ...
            let ps = (0..n)
                .map(|_| Particle { x: rf(&mut rng) * w as f32, y: rf(&mut rng) * h as f32, phase: rf(&mut rng) * TAU, rate: 0.6 + rf(&mut rng) * 0.8, amp: 3.0 + rf(&mut rng) * 5.0, var: (rf(&mut rng) * 4.0) as u8 })
                .collect();
        ...
        Atmos::Rain => {
            ...
            let ps = (0..n)
                .map(|_| Particle { x: rf(&mut rng) * w as f32, y: rf(&mut rng) * h as f32, phase: rf(&mut rng) * TAU, rate: 8.0 + rf(&mut rng) * 6.0, amp: 0.4 + rf(&mut rng) * 0.6, var: (rf(&mut rng) * 2.0) as u8 })
                .collect();
    ...
    match s.atmos {
        Atmos::Mist => {
            ...
            for y in y0..y1.min(gh) {
                ...
                for x in 0..gw {
                    let xf = x as f32;
                    let v = (xf * 0.07 + ts * 0.05 + seed).sin() * 0.5 + (xf * 0.023 - ts * 0.031 + seed * 2.0).sin() * 0.35 + (y as f32 * 0.9 + xf * 0.031).sin() * 0.25;
                    let d = (v + 0.5) * vert;
                    if d > 0.75 {
                        line[x] = Cell::new('▒', c1);
                    } else if d > 0.5 {
        ...
        Atmos::Rain => {
            for p in &s.parts {
                ...
                set(grid, gw, gh, (x.rem_euclid(gw as f32)) as i32, y as i32, ch, c);
            }
        }
        Atmos::Snow => {
            let glyphs = ['*', '·', '.'];
...
pub(crate) fn draw_fable_2_forest(grid: &mut Grid, width: usize, height: usize, seed: u64, palette: &[Color; 5], t: f32, k: &ForestKnobs) {
    ...
    let key = Key { w: gw, h: gh, seed, palette: *palette, density: k.density, layers: k.layers, hue: k.hue, atmos: k.atmos, haze: k.haze, moon: k.moon, energy: k.energy, ground: k.ground, fruit: k.fruit };
    ...
    SCENE.with(|c| {
        ...
        let lit: Vec<Color> = measure_layer("fable-2-forest", "light", || {
            let l = if animating { k.light * (TAU * ts / 48.0).sin() } else { 0.0 };
            s.pal
                .colors
                .iter()
                .map(|&c| if l >= 0.0 { lerp_color(c, lighten(c, 40), l) } else { lerp_color(c, darken(c, 40), -l) })
                .collect()
        });
        ...
        measure_layer("fable-2-forest", "sky", || {
            ...
            if let Some(m) = &s.moon {
                ...
                let mx = if animating { (m.x0 + ts * gw as f32 / 90.0).rem_euclid(span) - 2.0 * m.r - 2.0 } else { m.x0 };
                let r = m.r;
                for dy in (-(r as i32) - 1)..=(r as i32 + 1) {
                    let hw = 2.0 * ((r + 1.0) * (r + 1.0) - dy as f32 * dy as f32).max(0.0).sqrt();
                    for dx in (-(hw as i32))..=(hw as i32) {
                        ...
                        if x >= 0 && y >= 0 && (x as usize) < gw && (y as usize) < s.horizon.min(gh) && s.bg_ch[y as usize * gw + x as usize] == ' ' {
                            grid[y as usize][x as usize] = Cell::new(ch, c);
                        }
                    }
                }
        ...
        measure_layer("fable-2-forest", "atmos", || paint_atmos(grid, gw, gh, s, &lit, t, ts, animating));
    });
}
