//! fable-2-forest: layered forest of the fable-2 species with sway, drifting
//! atmosphere and a slow light cycle. Geometry is grown once per key and cached.

use crate::_0_profile::measure_layer;
use crate::color::*;
use crate::fable_2_trees::*;
...
impl ForestKnobs {
    ...
    pub(crate) fn from_env() -> Self {
        ForestKnobs {
            ...
            atmos: param_f32("ATMOS", 0.0).clamp(0.0, 6.0),
            haze: param_f32("HAZE", 0.5).clamp(0.0, 1.0),
            moon: param_f32("MOON", 1.0).clamp(0.0, 2.0),
...
fn build_scene(key: Key, k: &ForestKnobs) -> Scene {
    let w = key.w;
    let h = key.h;
    let mut rng = StdRng::seed_from_u64(key.seed ^ 0xF0B1_E5CA);
    let mut pal = Palette::new();
    let hue0 = (palette_hue(&key.palette) + key.hue as f64 + (rf(&mut rng) as f64 - 0.5) * 30.0)
        .rem_euclid(360.0);
    ...
    // ground line: two slow sines, amplitude a slice of the height
    let a1 = h as f32 * 0.02;
    let a2 = h as f32 * 0.01;
    let (f1, f2) = (
        0.9 / w as f32 * TAU * (0.6 + rf(&mut rng)),
        2.7 / w as f32 * TAU * (0.6 + rf(&mut rng)),
    );
    let (p1, p2) = (rf(&mut rng) * TAU, rf(&mut rng) * TAU);
    let gl: Vec<usize> = (0..w)
        .map(|x| {
            let v =
                horizon as f32 + a1 * (x as f32 * f1 + p1).sin() + a2 * (x as f32 * f2 + p2).sin();
            (v.round() as i32).clamp(1, h as i32 - 2) as usize
        })
        .collect();

    // backdrop planes: sky bands above the line, textured ground below
    ...
    for y in 0..h {
        for x in 0..w {
            ...
            if y < gl[x] {
                ...
                bg_ch[i] = if hash3(x as u32, 7, key.seed as u32) % 5 == 0 {
                    ' '
                } else {
                    '╌'
                };
    ...
    let star_slots = [
        pal.intern(hsl_to_rgb(sky_hue, 0.2, 0.55)),
        pal.intern(hsl_to_rgb(sky_hue, 0.2, 0.3)),
    ];
        ...
        for x in 0..w {
            if hash3(x as u32, y as u32, key.seed as u32 ^ 0x57A5) % 45 == 0 {
                stars.push((
                    x as u16,
                    y as u16,
                    (hash3(x as u32, y as u32, 9) % 250) as u8,
                ));
            }
        }
    }

    // moon: most seeds get one; MOON scales it, zero hides it
    let moon = if key.moon > 0.0 && rf(&mut rng) < 0.75 {
        let pale = key.palette[4];
        Some(Moon {
            x0: rf(&mut rng) * w as f32,
            y: horizon as f32 * (0.15 + rf(&mut rng) * 0.25),
            r: (h as f32 * 0.07 * key.moon).max(1.5),
            core: pal.intern(lighten(pale, 10)),
            rim: pal.intern(darken(pale, 60)),
            glow: pal.intern(hsl_to_rgb(sky_hue, 0.3, 0.16)),
        })
    } else {
        None
    };

    // species mix: one dominant, one secondary, the rest rare
    ...
    for li in 0..layers {
        ...
        while layer_trees.len() < n && attempts < n * 12 {
            ...
            let hue =
                (hue0 + (rf(&mut rng) as f64 - 0.5) * 40.0 + lfrac as f64 * 10.0).rem_euclid(360.0);
            let sat = 0.3 + 0.35 * lfrac as f64;
            let light = 0.18 + 0.22 * lfrac as f64;
            let ink =
                Ink::from_hue(hue, sat, light).fade(haze_color, key.haze * (1.0 - lfrac) * 0.7);
            let mut scratch = vec![vec![Cell::blank(); pw]; th + rd as usize];
            let mut trng = StdRng::seed_from_u64(
                key.seed ^ hash3(li as u32, layer_trees.len() as u32, 0x7EE) as u64,
            );
            grow_species(
                sp,
                &mut scratch,
    ...
    // undergrowth tufts on the back line and the front root line
    let tuft_slots = [
        pal.intern(hsl_to_rgb(hue0 + 10.0, 0.45, 0.26)),
        pal.intern(hsl_to_rgb(hue0 - 5.0, 0.4, 0.34)),
    ];
    ...
    // atmosphere: ATMOS picks a kind, zero leaves it to the seed
    let atmos = if key.atmos >= 1.0 {
        ATMOS_KINDS[(key.atmos.round() as usize - 1).min(5)]
    } else {
        ATMOS_KINDS[rng.random_range(0..6u32) as usize]
    };
    ...
    let (atm_slots, parts) = match atmos {
        Atmos::Mist => (
            [
                pal.intern(hsl_to_rgb(sky_hue, 0.25, 0.22)),
                pal.intern(hsl_to_rgb(sky_hue, 0.25, 0.3)),
                0,
            ],
            Vec::new(),
        ),
        Atmos::Fireflies => {
            let n = ((w * h) / 900).clamp(8, 600);
            let ps = (0..n)
                .map(|_| Particle {
                    x: rf(&mut rng) * w as f32,
                    y: horizon as f32 * 0.6
                        + rf(&mut rng) * (h as f32 * 0.95 - horizon as f32 * 0.6),
                    phase: rf(&mut rng),
                    rate: 0.12 + rf(&mut rng) * 0.25,
                    amp: 2.0 + rf(&mut rng) * 4.0,
                    var: 0,
                })
                .collect();
            (
                [
                    pal.intern(darken(accent, 60)),
                    pal.intern(accent),
                    pal.intern(lighten(accent, 40)),
                ],
                ps,
            )
        }
        Atmos::Leaves => {
            let n = ((w * h) / 1500).clamp(6, 500);
            let warm = hsl_to_rgb(hue0 + 40.0, 0.6, 0.45);
            let ps = (0..n)
                .map(|_| Particle {
                    x: rf(&mut rng) * w as f32,
                    y: rf(&mut rng) * h as f32,
                    phase: rf(&mut rng) * TAU,
                    rate: 0.6 + rf(&mut rng) * 0.8,
                    amp: 3.0 + rf(&mut rng) * 5.0,
                    var: (rf(&mut rng) * 4.0) as u8,
                })
                .collect();
            (
                [
                    pal.intern(darken(warm, 30)),
                    pal.intern(warm),
                    pal.intern(lighten(warm, 30)),
                ],
                ps,
            )
        }
        Atmos::Rain => {
            let n = ((w * h) / 400).clamp(20, 3000);
            let wet = hsl_to_rgb(sky_hue, 0.3, 0.42);
            let ps = (0..n)
                .map(|_| Particle {
                    x: rf(&mut rng) * w as f32,
                    y: rf(&mut rng) * h as f32,
                    phase: rf(&mut rng) * TAU,
                    rate: 8.0 + rf(&mut rng) * 6.0,
                    amp: 0.4 + rf(&mut rng) * 0.6,
                    var: (rf(&mut rng) * 2.0) as u8,
                })
                .collect();
            ([pal.intern(darken(wet, 30)), pal.intern(wet), 0], ps)
        }
        Atmos::Snow => {
            let n = ((w * h) / 700).clamp(10, 1500);
            let ps = (0..n)
                .map(|_| Particle {
                    x: rf(&mut rng) * w as f32,
                    y: rf(&mut rng) * h as f32,
                    phase: rf(&mut rng) * TAU,
                    rate: 0.8 + rf(&mut rng) * 0.8,
                    amp: 2.0 + rf(&mut rng) * 2.0,
                    var: (rf(&mut rng) * 3.0) as u8,
                })
                .collect();
            (
                [
                    pal.intern(darken(pale, 70)),
                    pal.intern(darken(pale, 20)),
                    0,
                ],
                ps,
            )
        ...
        Atmos::Birds => {
            ...
            for _ in 0..flocks {
                ...
                for i in 0..n {
                    ...
                    ps.push(Particle {
                        x: fx - k as f32 * 3.0,
                        y: fy + sgn * k as f32 * (if i == 0 { 0.0 } else { 1.0 }),
                        phase,
                        rate,
                        amp: rf(&mut rng) * TAU,
                        var: 0,
            ...
            (
                [
                    pal.intern(darken(pale, 90)),
                    pal.intern(darken(pale, 50)),
                    0,
                ],
                ps,
            )
    ...
    let band_h = ((h as f32 * 0.07) as usize).max(2);
    let band_c = horizon + (h - horizon) / 6;
    let mist_band = (band_c.saturating_sub(band_h), (band_c + band_h).min(h));
...
fn paint_atmos(
    ...
    match s.atmos {
        Atmos::Mist => {
            let (y0, y1) = s.mist_band;
            let mid = (y0 + y1) as f32 * 0.5;
            let half = ((y1 - y0) as f32 * 0.5).max(1.0);
            let seed = s.key.seed as f32 * 0.37;
            for y in y0..y1.min(gh) {
                let vert = 1.0 - ((y as f32 - mid) / half).abs();
                let line = &mut grid[y];
                for x in 0..gw {
                    let xf = x as f32;
                    let v = (xf * 0.07 + ts * 0.05 + seed).sin() * 0.5
                        + (xf * 0.023 - ts * 0.031 + seed * 2.0).sin() * 0.35
                        + (y as f32 * 0.9 + xf * 0.031).sin() * 0.25;
                    let d = (v + 0.5) * vert;
                    if d > 0.75 {
                        line[x] = Cell::new('▒', c1);
                    } else if d > 0.5 {
        ...
        Atmos::Fireflies => {
            for p in &s.parts {
                let b = if animating {
                    0.5 + 0.5 * (TAU * (p.rate * ts + p.phase)).sin()
                } else {
                    0.5 + 0.5 * (TAU * p.phase).sin()
                };
                if b < 0.35 {
                    continue;
                }
                let x = p.x + p.amp * (ts * 0.2 + p.phase * 3.0).sin();
                let y = p.y + p.amp * 0.4 * (ts * 0.13 + p.phase * 5.0).sin();
                let (ch, c) = if b < 0.6 {
                    ('·', c0)
                } else if b < 0.85 {
                    ('•', c1)
        ...
        Atmos::Rain => {
            for p in &s.parts {
                ...
                let x = p.x + ts * p.amp * 0.3;
                let ch = if p.var == 0 { '|' } else { ':' };
                let c = if p.var == 0 { c1 } else { c0 };
                set(
                    grid,
                    gw,
                    gh,
                    (x.rem_euclid(gw as f32)) as i32,
                    y as i32,
                    ch,
                    c,
                );
...
#[tracing::instrument(level = "trace", target = "ascii_renderer::functions", skip_all)]
pub(crate) fn draw_fable_2_forest(
    grid: &mut Grid,
    width: usize,
    height: usize,
    seed: u64,
    ...
    if !hit {
        let scene = measure_layer("fable-2-forest", "build", || build_scene(key.clone(), k));
    ...
    SCENE.with(|c| {
        ...
        let lit: Vec<Color> = measure_layer("fable-2-forest", "light", || {
            let l = if animating {
                k.light * (TAU * ts / 48.0).sin()
            } else {
                0.0
            };
            s.pal
                ...
                .map(|&c| {
                    if l >= 0.0 {
                        lerp_color(c, lighten(c, 40), l)
                    } else {
                        lerp_color(c, darken(c, 40), -l)
                    }
                })
                .collect()
        });
        measure_layer("fable-2-forest", "backdrop", || {
            for y in 0..gh {
                let line = &mut grid[y];
        ...
        measure_layer("fable-2-forest", "sky", || {
            for &(x, y, ph) in &s.stars {
                let tw = if animating {
                    (ts * 0.7 + ph as f32 * 0.1).sin()
                } else {
                    (ph as f32 * 0.1).sin()
                };
                let c = if tw > 0.2 {
                    lit[s.star_slots[0] as usize]
                } else {
                    lit[s.star_slots[1] as usize]
                };
                grid[y as usize][x as usize] = Cell::new('·', c);
            }
            if let Some(m) = &s.moon {
                let span = gw as f32 + 4.0 * m.r + 4.0;
                let mx = if animating {
                    (m.x0 + ts * gw as f32 / 90.0).rem_euclid(span) - 2.0 * m.r - 2.0
                } else {
                    m.x0
                };
                let r = m.r;
                for dy in (-(r as i32) - 1)..=(r as i32 + 1) {
                    ...
                    for dx in (-(hw as i32))..=(hw as i32) {
                        ...
                        let x = mx.round() as i32 + dx;
                        let y = m.y.round() as i32 + dy;
                        if x >= 0
                            && y >= 0
                            && (x as usize) < gw
                            && (y as usize) < s.horizon.min(gh)
                            && s.bg_ch[y as usize * gw + x as usize] == ' '
                        {
                            grid[y as usize][x as usize] = Cell::new(ch, c);
                        }
                    }
        ...
        measure_layer("fable-2-forest", "trees", || {
            let tick = (t * 1.5) as u32;
            for tr in &s.trees {
                let amp = k.sway * (0.3 + 0.7 * tr.lfrac) * tr.sprite.root_row as f32 * 0.07;
                let wind = 0.6 * (TAU * ts / 22.0 + tr.phase).sin()
                    + 0.4 * (TAU * ts / 9.5 + tr.phase * 1.7).sin();
                let sway = if animating { amp * wind } else { 0.0 };
                let flicker = if animating {
                    0.1 + 0.1 * wind.abs()
                } else {
                    0.0
                };
        ...
        measure_layer("fable-2-forest", "undergrowth", || {
            let glyphs = [',', '"', ';', 'w', '∙'];
            for &(x, y, slot, ph) in &s.tufts {
                if y < 0 || y >= gh as i32 || x < 0 || x >= gw as i32 {
                    continue;
                }
                let idx = if animating {
                    (((ts * 1.2 + ph as f32 * 0.05).sin() * 0.5 + 0.5) * 4.99) as usize
                } else {
                    (ph as usize) % 5
                };
        ...
        measure_layer("fable-2-forest", "atmos", || {
            paint_atmos(grid, gw, gh, s, &lit, t, ts, animating)
        });
    });
}

#[tracing::instrument(level = "trace", target = "ascii_renderer::functions", skip_all)]
pub(crate) fn cli_fable_2_forest(
    mut grid: Grid,
    width: usize,
    height: usize,
    seed: u64,
    ...
    draw_fable_2_forest(&mut grid, width, height, seed, &palette, t_anim, &k);
    (grid, false)
}
...
mod tests {
    ...
    fn run(w: usize, h: usize, seed: u64, t: f32) -> String {
        ...
        draw_fable_2_forest(&mut g, w, h, seed, &p, t, &k);
        g.iter()
    ...
    fn every_atmosphere_renders() {
        for a in 1..=6u32 {
            ...
            let mut k = ForestKnobs::from_env();
            k.atmos = a as f32;
            draw_fable_2_forest(&mut g, 80, 24, 3, &p, 4.0, &k);
