//! Ignored release-mode bench for hyperhex-5.
//! Run: CARGO_BUILD_JOBS=4 cargo test --release -- --ignored --nocapture modes::_131_hyperhex_5
...
fn bench() {
    for &(w, h) in &[(200usize, 60usize), (400usize, 120usize)] {
        println!("hyperhex-5 {w}x{h} checksum: {:016x}", checksum(w, h));
        let (ms, layers) = bench_size(w, h);
        println!("hyperhex-5 {w}x{h}: {ms:.3} ms/frame");
        for l in &layers {
