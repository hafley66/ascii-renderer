...
thread_local! {
    static TILING_CACHE: RefCell<Option<((u32, u64, u32, u32), Rc<Vec<Tile>>)>> =
        const { RefCell::new(None) };
}

/// The reflection walk depends only on (depth, skew, min_edge, scale), none of
/// which change between frames, so cache it per thread and reuse the `Rc`.
fn tiling_cached(depth: u32, skew: f64, min_edge: f32, s: f32) -> Rc<Vec<Tile>> {
    TILING_CACHE.with(|c| {
        let mut c = c.borrow_mut();
        let key = (depth, skew.to_bits(), min_edge.to_bits(), s.to_bits());
        if let Some((k, v)) = c.as_ref() {
            if *k == key {
                return v.clone();
            }
        }
        let v = Rc::new(build_tiling(depth, skew, min_edge, s));
        *c = Some((key, v.clone()));
        v
    })
}
...
fn draw(frame: &mut ModeFrame<'_>, k: &Knobs) {
    ...
    let tiles = measure_layer(NAME, "topology", || {
        tiling_cached(k.depth, k.skew, 1.1, s)
    });
    ...
    measure_layer(NAME, "ground", || {
        ...
        let fg_dust = lerp_color(pal[0], pal[1], 0.5);
        let zx2: Vec<f32> = (0..w)
            .map(|x| {
                let z = (x as f32 + 0.5 - cx0) / s;
                z * z
            })
            .collect();
        let zy2: Vec<f32> = (0..h)
            .map(|y| {
                let z = (y as f32 + 0.5 - cy0) / (0.5 * s);
                z * z
            })
            .collect();
        for y in 0..h {
            ...
            for x in 0..w {
                ...
                if d > 1.0 {
                    ...
                    let dust = unit(seed, 10, (row + x) as u64) > 0.992;
                    let ch = if dust { '.' } else { ' ' };
                    grid[y][x] = Cell::with_bg(ch, fg_dust, bg);
                }
            }
        }
    });
...
mod bench {
    ...
    //! CARGO_BUILD_JOBS=4 cargo test --release -- --ignored --nocapture modes::_115_hyperhex_3_
    use super::*;
