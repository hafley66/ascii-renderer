...
fn draw_tideglass(frame: &mut ModeFrame<'_>, k: &[f32; 14], t: f32) {
    ...
    measure_layer("tideglass", "backdrop", || {
    ...
    measure_layer("tideglass", "cables", || {
        for side in [-1.0, 1.0] {
    ...
    measure_layer("tideglass", "orbital_bands", || {
        for front in [false, true] {
            for band in 0..rings {
                ...
                for j in 0..samples {
                    ...
                    let color =
                        lerp_color(ink, base, if front { 0.7 + 0.3 * a.sin() } else { 0.32 });
                    line(
                        frame.grid,
                        project(a, r),
    ...
    measure_layer("tideglass", "pendants", || {
    ...
    measure_layer("tideglass", "reflection", || {
        ...
        for (depth, row) in sea.iter_mut().enumerate() {
            ...
            for (x, cell) in row.iter_mut().enumerate() {
                let wave =
                    (depth as f32 * 1.7 - t * 1.8 + (x as f32 * 0.08 + t * 0.4).sin()).sin();
