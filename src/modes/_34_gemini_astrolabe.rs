                    ...
                    if atten * params.rays > 0.4 {
                        ...
                        if ray_wave > 0.0 {
                            ...
                            if ray_power > 0.4 {
                                ...
                                let r_fg =
                                    lerp_color(cell_fg, palette[4], (ray_power * 0.4).min(0.9));
                                cell_ch = r_ch;
                                cell_fg = r_fg;
                            }
                        }
            ...
            for gear in &gear_defs {
                ...
                for sp in 0..gear.spokes {
                    ...
                    for step in 1..=sp_steps {
                        ...
                        let gz = -scale * 0.6;
                        let (rx, ry, rz) =
                            rotate_3d(gx, gy, gz, params.tilt * 0.6, anim_t * 0.1, 0.0);
                        if let Some((px, py, pz)) = project(rx, ry, rz, cx, cy, aspect, fov) {
    ...
    measure_layer("gem-aetherium", "orrery", || {
        ...
        for planet in &planet_defs {
            ...
            for ts in 0..trace_steps {
                ...
                let oz = 0.0;
                let (tx, ty, tz) = rotate_3d(
                    ox,
                    oy,
                    oz,
                    planet.inclination,
                    planet.node_angle + anim_t * 0.05,
                    0.0,
                );
        ...
        if params.comets > 0 {
            ...
            for comet in &comet_defs {
                ...
                let true_anomaly = 2.0
                    * (((1.0 + comet.eccentricity).sqrt() * (ecc_anomaly * 0.5).sin())
                ...
                if let Some((px, py, pz)) = project(hw_x, hw_y, hw_z, cx, cy, aspect, fov) {
                    ...
                    for tail_step in 1..=comet.tail_len {
                        ...
                        let t_true = 2.0
                            * (((1.0 + comet.eccentricity).sqrt() * (t_ecc * 0.5).sin())
