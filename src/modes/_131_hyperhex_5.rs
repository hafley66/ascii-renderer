...
pub(super) struct Hyperhex5;
pub(super) static MODE: Hyperhex5 = Hyperhex5;

const NAME: &str = "hyperhex-5";
const HELP: &str = "hyperhex-5: hyperbolic {6,6} tiling in four acts [depth] [skew] [spin] [breath] [swell] [passage] [fill] [wound] [act] [ghost] [horizon]";
...
mod tests {
    ...
    #[test]
    fn snapshot_80x24() {
        insta::assert_snapshot!("hyperhex_5_80x24", render_at(80, 24, 42, 0.0));
    }

    #[test]
    fn snapshot_t6() {
        insta::assert_snapshot!("hyperhex_5_t6", render_at(80, 24, 42, 6.0));
    }

    #[test]
    fn snapshot_wound() {
        insta::assert_snapshot!("hyperhex_5_wound", render_at(80, 24, 42, 1.0));
    }

    #[test]
    fn snapshot_eclipse() {
        insta::assert_snapshot!("hyperhex_5_eclipse", render_at(80, 24, 42, 1.5));
    }
    ...
    fn frame_cost() {
        ...
        assert!(
            avg_ms < 6.0,
            "hyperhex-5 average frame {avg_ms:.3} ms exceeds 6 ms"
        );
    }
}

#[cfg(test)]
#[path = "_131_hyperhex_5_bench.rs"]
mod bench;
