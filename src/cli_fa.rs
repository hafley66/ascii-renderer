...
pub(crate) fn cli_fa4_fullmetal_alchemist4(
    ...
    let draw_curve = |grid: &mut Grid,
     ...
     -> Vec<(i32, i32)> {
        ...
        for i in 0..=samples {
            ...
            let u = 1.0 - t;
            let x = (u * u * p0.0 as f32 + 2.0 * u * t * p1.0 as f32 + t * t * p2.0 as f32).round()
                as i32;
            let y = (u * u * p0.1 as f32 + 2.0 * u * t * p1.1 as f32 + t * t * p2.1 as f32).round()
                as i32;
    ...
    let draw_diamond_frame = |grid: &mut Grid, cx: i32, cy: i32, rx: i32, ry: i32, fg: Color| {
        let top = (cx, cy - ry);
        let right = (cx + rx, cy);
        let bottom = (cx, cy + ry);
        let left = (cx - rx, cy);
    ...
    for path in 0..path_count {
        ...
        for (ri, &(scale, _, ring_phase, ring_color)) in ring_specs.iter().enumerate() {
            let wobble = (path as f32 * 0.73 + ri as f32 * 1.11 + seed as f32 * 0.013).sin() * 0.22;
...
pub(crate) fn cli_fa5_fullmetal_alchemist5(
    ...
    for r in 0..poly_count {
        ...
        let phase = base_phase + r as f32 * 0.41 + rng.random_range(-0.20..0.20);
